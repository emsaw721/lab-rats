...
const formFormula = document.getElementById("formula"); // formular input
const formDigits = document.getElementById("digits"); //how many digits of results
const calculateButton = document.getElementById("calculateButton"); //calculate button
const results = document.getElementById("results"); //result output
const examples = document.getElementById("examples"); //examples that you can choose to calculate
...
function calculateMolecule(formula) {
  ...
  const validFormula =
    /([A-Z][a-z]{0,2}\d*)+|\(|\)\d*|\.\d*([A-Z][a-z]{0,2}\d*)+/g;
  ...
  formula = formula.replace(/\s/g, ""); //remove space
  chemArray.forEach((x) => {
...
function showResults(formula, digits) {
  ...
  for (let [ele, atoms] of Object.entries(elements)) {
    ...
    allResults +=
      `${atoms} ${ele} * ${fix(atom[ele])} = ${fix(eltotal)} (${fix(
        (eltotal / total) * 100
      )}% of mass)` + "\n";
  }
...
//choose examples to the input box
...
calculateButton.addEventListener("click", function () {
  results.value =
    showResults(formFormula.value, formDigits.value) + "\n" + results.value;
});
